...
class WC_Reepay_Api {
	 ...
	 * @return array|WP_Error
	 */
	public function charge( WC_Order $order, $token, $amount, $currency ) {
		...
		$params = array(
			...
			'amount'    => rp_prepare_amount( $amount, $currency ),
			'currency'  => $currency,
			'source'    => $token,
	...
	public function settle( WC_Order $order, $amount = null, $item_data = false ) {
		...
		if ( $order_data['authorized_amount'] === $order_data['settled_amount'] ) {
			$message = sprintf(
				__( 'Payment has been settled. Transaction: %s', 'reepay-checkout-gateway' ),
				$result['transaction']
			);

			WC_Reepay_Order_Statuses::set_settled_status( $order, $message, $result['transaction'] );
		} else {
			$message = sprintf(
